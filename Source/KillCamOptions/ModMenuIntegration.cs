    ...
    public class ModMenuIntegration : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (!Directory.Exists(iniLocation))
            {
                ...
                Debug.Log("[LLBMM] KillCamOptions: Created ModSettings Folder");
            }
            if (!File.Exists(iniLocation + gameObject.name + ".ini"))
            {
                InitConfig();
                Debug.Log($"[LLBMM] KillCamOptions: {gameObject.name}.ini Created in {iniLocation}");
                return;
            }
            ReadIni();
        }
        ...
        private void InitConfig()
        {
            ...
            */
            ...
            AddToWriteQueue("(bool)killCameraIs", "true");
            AddToWriteQueue("(slider)playKillCameraOverSpeed", "40|40|240");
